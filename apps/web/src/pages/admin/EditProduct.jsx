...
import { handleEditProduct, handleRequiredField, validationFormProduct } from "../../helpers/dashboard/manageProduct";
				...
				<form className={`p-3 ${isLoading && 'hidden'}`}>
					...
					<div className='grid gap-2 mb-3'>
						...
						<div className='flex disabled:cursor-not-allowed disabled:opacity-50 ' >
							...
							<div>
								<select className='border rounded-r-lg bg-gray-200 p-2.5' value={data.unit} onChange={(e) => setData({ ...data, unit: e.target.value })}>
					...
					<div className='mt-10 flex gap-16 justify-center'>
						<Button theme={customButton} color='primary' onClick={() => validationFormProduct(file, data, setError, error, setRequiredField, requiredField, setIsOpen)}>Save</Button>
						<Button theme={customButton} color='secondary' onClick={() => navigate('/manage/product', { replace: true })}>Cancel</Button>
					</div>
				</form>
				<ModalConfirm
					show={isOpen}
					header={'Edit Product'}
					message={'Are you sure want to edit product?'}
					onClose={() => setIsOpen(false)}
					onConfirm={() => { setIsOpen(false); handleEditProduct(setIsLoading, data, prevName, customToast, deletedImage, uploadImage, navigate); }}
				/>
			</div>
		</div>
	</LayoutDashboard>
